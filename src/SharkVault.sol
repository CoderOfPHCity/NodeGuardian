...
contract SharkVault {
    ...
    uint256 public constant INTEREST_RATE_PERCENT = 1;

    IERC20 public immutable gold;
    IERC20 public immutable seagold;
    ...
    constructor(IERC20 _gold, IERC20 _seagold) {
        gold = _gold;
        seagold = _seagold;
    }
     ...
     */
    function updatedAccount(address _accountOwner) public view returns (LoanAccount memory account) {
        account = accounts[_accountOwner];

        if (account.borrowedSeagold > 0) {
            uint256 blockDelta = block.number - account.lastBlock;
            uint256 interest = account.borrowedSeagold * blockDelta * INTEREST_RATE_PERCENT / 100;

            account.depositedGold = (account.depositedGold >= interest) ? account.depositedGold - interest : 0;
        }
